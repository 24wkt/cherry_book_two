...
#「ブロックで初期値を指定しないと思わぬ不具合を作ってしまう」と説明すると、今度は「全部ブロックを使えばいいんだ」と考えてしまう人もいるかもしれません。
#ですが、ブロックを使わなくても問題が発生しないケースもあります。

#Rubyに限らず、プログラミングの世界にはミュータブル(mutable)とイミュータブル(immutable)という言葉が登場します。

#【ミュータブル(mutable)】
# ==> ミュータブルは「変更可能な」という意味で、ミュータブルなオブジェクトには破滅的な変更が適用できます。
#     そのためm配列の要素が同じオブジェクトを参照していると、破滅的な変更によって本来変わってほしくない値まで一緒に変わってしまう恐れがあります。
#     Rubyの文字列、stringクラスはミュータブルなので先程の説明のように、ブロックを使わないと思わぬ不具合の原因となります。

#【イミュータブル(immutable)】
# ==> イミュータブルは「変更できない、不変の」という意味で、イミュータブルなオブジェクトには破滅的な変更は適用できません。
#     そのため、ブロックを使わずに初期値を設定してもm文字列で発生したような不具合は発生しないことになります。

#Rubyには常にイミュータブルなクラスや値がいくつかあります。基本的なデータ型でいうと、次の４種類です。
...
#この考えに従うと、次のようなコードはブロックを使わなくてもほかの要素が一緒に変更される問題が起こりません。(negative!は実在しないメソッド)
  ...
  # 数値だと破滅的な変更(例えば強制的に負の数に変更するなど)はできない
  n.negative!

#使用するオブジェクトによって考え方を変えなければいけないのは少し大変ですが実際にプログラムを書いていくと、
#「うっかり破滅的な変更を実行してしまったために不具合を作ってしまった」ということが時々起こります、これも大事な考え方の１つなのでしっかり理解しておきましょう！
#なお、文字列のようなミュータブルなオブジェクトはfreezeメソッドを使って、変更不可にすることもできます。
