...
#[4.4.5]&とシンボルを使ってもっと簡素に書く
#ここからは少し上級テクニックになりますが、ブロックを使うメリットは条件によってかなり簡素に書くことができます。

  #このコードは、
  puts ['ruby', 'java', 'perl'].map { |s| s.upcase }  # ==> RUBY, JAVA, PERL
  #こう書き換えられる
  puts ['ruby', 'java', 'perl'].map(&:upcase)   # ==> RUBY, JAVA, PERL
...
#mapメソッドやselectメソッドにブロックを渡す代わりに、「&:メソッド名」という引数を渡しています。
#この書き方は次の条件が揃った時に使うことができます。
